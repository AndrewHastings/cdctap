...
/* read if fname=NULL, else write w/actual file name returned in fname */
TAPE *tap_open(char *path, char *fname)
{
	FILE *fp;
	TAPE *rv;
	int i;
	...
	if (fname) {
		...
		for (i = 0; i < 100; i++) {
			...
			sprintf(fname, "%s.%d.tap", path, i+1);
		}
	} else {
		fp = fopen(path, "r");
	}
	...
	if (rv) {
		rv->tp_fp = fp;
		rv->tp_path = fname ? fname : path;
		rv->tp_buf = NULL;
		rv->tp_nbytes = 0;
		rv->tp_status = fname ? TP_WRITE : 0;
	}
